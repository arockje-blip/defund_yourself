...
# 1. New Enemy Logic: Check for Troops first
$enemyLogic = @'
    gameState.enemyAttacks.forEach((a, index) => {
        ...
        if (Math.hypot(canvas.width/2 - a.x, canvas.height/2 - a.y) < 100) {
            gameState.enemyAttacks.splice(index, 1); gameState.health -= 5;
        }
    });
